...
function chartQuestion(){
    inquirer.prompt([
        ...
        type:"list",
        choices:["Search by artist", "Search song title", "Display multi-hit artists", "Search by chart position", "find top hit and albums", "Quit"]
    },
    {
        name:'artistAns',
    ...
    {
        name:'combineAns',
        message:"Which artist to check?",
        type:"prompt",
        when:function(answers){
            if(answers.choice==="find top hit and albums"){
                return true
            } else {
                return false
            }
        ...
        else if(answers.choice === "find top hit and albums"){
            tableCombineCheck(answers.combineAns);
        }
...
function findRepeats(){
    connection.query(`SELECT artist, count(*) as "Popular tracks" FROM top5000 GROUP BY artist HAVING COUNT(*)>1 order by count(*);`, function(err, res) {
        if (err) {
            throw err;
        }
        else{
...
function tableCombineCheck(artist){
    connection.query(`SELECT s.year, s.artist, s.title, a.album FROM top5000 AS s INNER JOIN top_albums AS a ON s.year = a.year WHERE s.artist = a.artist AND s.artist = "${artist}" ORDER BY  s.year ASC;`, function(err, res) {
        if (err) {
            throw err;
        }
        else{
